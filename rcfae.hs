...
interp (App funExpr argExpr) env =
    ...
    in (funVal argVal)
interp (Rec name (Fun arg funBody) recBody) env =
    let extended = extend env
                          name
                          (FunV (\ val -> interp funBody (extend extended arg val)))
    in interp recBody extended
interp (If0 pred truth falsity) env =
    let NumV val = interp pred env
    in
        if val == 0
        then interp truth env
        else interp falsity env
